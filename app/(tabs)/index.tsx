...
type GpsPoint = {
  ...
  hk_hr: number|null; hk_hrv: number|null; hk_temp_c: number|null;
  hk_spo2: number|null; hk_cal: number|null;
};
...
export default function WalkLoggerScreen() {
  ...
  const tryLiDAR = useCallback(() => {
    try { const L = (global as any).WalkLoggerLiDAR; if (!L) return; L.startSession((a: number) => setLidarAlt(a)); } catch(_) {}
  }, []);
      ...
      try {
        ...
        if (info.exists) {
          ...
          if (saved?.points?.length > 0) {
            Alert.alert("Unsaved walk found", `${saved.points.length} points from ${saved.autosaved_at}. Recover?`,
  ...
  const syncTrailFromFile = async () => {
    try {
      ...
      const points = (await FileSystem.readAsStringAsync(LOG_FILE)).trim().split("\n")
        .filter(Boolean).map(l => { try { return JSON.parse(l) as GpsPoint; } catch { return null; } })
  ...
  const startAutosave = (n: string, tt: TripType) => {
    ...
    autosaveRef.current = setInterval(async () => {
      try { await FileSystem.writeAsStringAsync(AUTOSAVE_FILE, JSON.stringify({
    ...
    fgWatchRef.current = await Location.watchPositionAsync(
      ...
      (loc) => {
        ...
        const p: GpsPoint = {
          ...
          altitude: loc.coords.altitude ?? null, accuracy: loc.coords.accuracy ?? null,
          speed_mps: loc.coords.speed ?? null, heading: loc.coords.heading ?? null,
          heart_rate: null, wind_speed_mps: w ? w.wind_kph / 3.6 : null,
          ...
          hk_hr: hs.heart_rate, hk_hrv: hs.hrv, hk_temp_c: hs.wrist_temp_c,
          hk_spo2: hs.spo2, hk_cal: hs.calories_active,
        };
        setCurrentPos(p);
  ...
  const handleStart = async () => {
    try {
      ...
      if (currentPos) {
        ...
        weatherPoll.current = setInterval(() => fetchWeather(currentPos.latitude, currentPos.longitude), 30000);
      }
      setRunning(true);
  ...
  const handleStop = async () => {
    try {
      ...
      setRunning(false); await syncTrailFromFile();
  ...
  const exportGeoJSON = async (points: GpsPoint[], expNote: string, expType: string, expElapsed: number) => {
    ...
    const geojson = {
      ...
      properties: { session: sessionName, points: points.length, duration_sec: expElapsed,
        ...
        sensors_active: Object.entries(sensorAvail).filter(([,v]) => v).map(([k]) => k) },
      features: [
        { type: "Feature",
        ...
        ...points.map((p, i) => ({
          ...
          properties: {
            ...
            speed_mps: p.speed_mps, heading: p.heading, accuracy: p.accuracy,
  ...
  const windStr     = weather ? `${weather.wind_kph}km/h ${windDir}` : "--";
  const gustStr     = weather && weather.gust_kph > weather.wind_kph + 5 ? ` G${weather.gust_kph}` : "";
  const tempStr     = weather ? `${weather.temp_c}C` : "--";
  const baroStr     = currentPos?.pressure_hpa ? `${currentPos.pressure_hpa.toFixed(0)}hPa` : null;
  ...
  const hrDisplay   = hkData.heart_rate !== null ? `${hkData.heart_rate}` : "--";
  const hrLabel     = hkData.heart_rate !== null ? "BPM" : "ALT";
  const hrValue     = hkData.heart_rate !== null ? hrDisplay : altStr;
  const sensorDots  = [
    { label: "A", key: "accelerometer" }, { label: "G", key: "gyroscope" },
    { label: "M", key: "magnetometer" }, { label: "D", key: "deviceMotion" }, { label: "B", key: "barometer" },
  ] as const;
  ...
  return (
    <View style={s.container}>
      ...
      <View style={s.weatherBar}>
        <Text style={s.wi}>TEMP {tempStr}</Text>
        <Text style={s.wi}>WIND {windStr}{gustStr}</Text>
        {baroStr && <Text style={s.wi}>BARO {baroStr}</Text>}
        {lidarAlt !== null && <Text style={s.wi}>LIDAR {lidarAlt.toFixed(1)}m</Text>}
        {hkData.hrv !== null && <Text style={s.wi}>HRV {Math.round(hkData.hrv)}ms</Text>}
        <View style={s.dots}>
      ...
      <View style={s.statsBar}>
        <View style={s.stat}><Text style={s.sv}>{fmt(elapsed)}</Text><Text style={s.sl}>TIME</Text></View>
        ...
        <View style={s.stat}><Text style={s.sv}>{hrValue}</Text><Text style={s.sl}>{hrLabel}</Text></View>
      </View>
      ...
      <TextInput style={s.noteInput} placeholder="Session note" placeholderTextColor="#444"
...
const s = StyleSheet.create({
  ...
  weatherBar:  { flexDirection: "row", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap",
                 backgroundColor: "#0D1117", paddingVertical: 5, paddingHorizontal: 12, borderTopWidth: 1, borderTopColor: "#1A2030" },
  wi:          { color: "#A0B8D0", fontSize: 11, fontWeight: "500", marginRight: 6 },
